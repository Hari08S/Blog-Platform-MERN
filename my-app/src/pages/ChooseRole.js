...
function ChooseRole({ user, onLogout }) {
  ...
  const [stats, setStats] = useState(() => {
    if (user?.email) {
      try {
        const cached = localStorage.getItem(`dashboard_stats_${user.email}`);
      ...
      axios.get(`https://blog-platform-erux.onrender.com/dashboard-stats?email=${user.email}`)
        .then(res => {
          if (res.data.success) {
            ...
            localStorage.setItem(`dashboard_stats_${user.email}`, JSON.stringify(res.data.stats));
          }
        })
    ...
    <div className="cr-container">
      ...
      <div className="cr-content">
        ...
        <div className="cr-stats-grid">
          <div className={`cr-stat-card ${!stats ? "cr-skeleton" : ""}`}>
            <div className="cr-stat-icon" style={{background:"rgba(102,126,234,0.15)"}}><Create style={{color:"#667eea"}}/></div>
            <div className="cr-stat-value">{stats ? stats.totalBlogs : <span className="cr-shimmer"></span>}</div>
          ...
          <div className={`cr-stat-card ${!stats ? "cr-skeleton" : ""}`}>
            ...
            <div className="cr-stat-value">{stats ? stats.totalLikes : <span className="cr-shimmer"></span>}</div>
          ...
          <div className={`cr-stat-card ${!stats ? "cr-skeleton" : ""}`}>
            ...
            <div className="cr-stat-value">{stats ? stats.totalComments : <span className="cr-shimmer"></span>}</div>
          ...
          <div className={`cr-stat-card ${!stats ? "cr-skeleton" : ""}`}>
            ...
            <div className="cr-stat-value">{stats ? stats.totalViews : <span className="cr-shimmer"></span>}</div>
